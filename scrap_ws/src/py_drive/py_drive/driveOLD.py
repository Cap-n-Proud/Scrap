...
class Robot(Node):
    ...
    def move(self, x, y):
        ...
        if self.simulation:
            self.get_logger().info(
                ...
                + str(speedL)
                + " signal to robot: "
                + str(max(-1, min(1, speedL)))
            )
            self.get_logger().info(
                'Right speed se to: "%s"'
                + str(speedR)
                + " signal to robot: "
                + str(max(-1, min(1, speedR)))
            )
        else:
            self.kit.motor1.throttle = speedL  # max(-1, min(1, speedL))
            self.kit.motor2.throttle = -speedR  # max(-1, min(1, speedR))
    ...
    def joy_topic(self, msg):
        ...
        if abs(msg.axes[0]) >= 0.05 or abs(msg.axes[1]) >= 0.05:
        ...
        if msg.axes[7] != 0:
            self.kit.motor1.throttle = msg.axes[7] * self.speed_limit - self.left_trim
            self.kit.motor2.throttle = -msg.axes[7] * self.speed_limit - self.right_trim
        if msg.axes[6] != 0:
            self.kit.motor1.throttle = msg.axes[6] * self.speed_limit - self.left_trim
            self.kit.motor2.throttle = msg.axes[6] * self.speed_limit - self.right_trim
        if msg.buttons[2] == 1:
